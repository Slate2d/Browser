    ...
    try:
        while True:
            ...
            if payload.get("type") == "heartbeat":
                # Update DB state
                with db() as conn:
                    conn.execute("UPDATE profiles SET state=?, last_url=? WHERE id=?",
                                 (payload.get("state","running"), payload.get("url",""), payload["profile_id"]))                
                # Rebroadcast to UI clients
...
def create_profile(payload: ProfileCreate):
    with db() as conn:
        cur = conn.execute("INSERT INTO profiles(id, name, proxy, state) VALUES(hex(randomblob(16)), ?, ?, 'stopped') RETURNING id",
...
def _worker_cmd(profile_id: str, name: str, proxy: str):
    py = sys.executable
    ws = f"ws://127.0.0.1:8000{WS_PATH}"
    profile_dir = (PROFILES_DIR / profile_id).as_posix()
    return [py, (BASE_DIR / "worker.py").as_posix(), "--id", profile_id, "--name", name, "--proxy", proxy, "--ws", ws, "--dir", profile_dir]
